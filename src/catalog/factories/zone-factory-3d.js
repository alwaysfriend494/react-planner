import {
  ...
  export function createZone(element, layer, scene, textures) {
    let vertices = [];
    ...
    if (element.selected) {
      color = SharedStyle.ZONE_MESH_COLOR.selected;
    } else if (textureName && textureName !== 'none') {
      color = SharedStyle.ZONE_MESH_COLOR.unselected;
    }
    ...
    let zoneMaterial = new MeshPhongMaterial({side: DoubleSide, color});
  
    /* Create holes for the zone */
    element.holes.forEach(holeID => {
      let holeCoords = [];
      layer.getIn(['zones', holeID, 'vertices']).forEach(vertexID => {
    ...
    applyTexture(zoneMaterial, texture, width, height);
  
    let zone = new Mesh(shapeGeometry, zoneMaterial);
  ...
  export function updatedZone( element, layer, scene, textures, mesh, oldElement, differences, selfDestroy, selfBuild ) {
    let noPerf = () => { selfDestroy(); return selfBuild(); };
    let floor = mesh.getObjectByName('floor');
  
    if( differences[0] == 'selected' ) {
      let color = element.selected ? SharedStyle.ZONE_MESH_COLOR.selected : ( element.properties.get('patternColor') || SharedStyle.ZONE_MESH_COLOR.unselected );
